...
class Carrito {
    ...
    public function cancelarVenta($folio, $motivo) {
        ...
        try {
            ...
            $stmt = $conn->prepare(
                "SELECT dv.producto_id, dv.cantidad, p.stock_actual, p.nombre
            ...
            $stmt  = $conn->prepare(
                "INSERT INTO cancelaciones_venta (venta_id, folio_venta, motivo, monto_cancelado)
                 VALUES (?, ?, ?, ?)"
            );
    ...
    public function buscarVentaPorFolio($termino) {
        ...
        $termino = substr(trim($termino), 0, 50);

        if (empty($termino)) {
            return ['success' => false, 'message' => 'Ingrese un folio o parte de él para buscar'];
        }

        // Limpiar el término: sólo alfanuméricos y guiones
        $termino_limpio = preg_replace('/[^a-zA-Z0-9\-]/', '', $termino);

        if (empty($termino_limpio)) {
            return ['success' => false, 'message' => 'Término de búsqueda inválido'];
        }
        ...
        try {
            ...
            $stmt = $conn->prepare(
                "SELECT v.*,
                        GROUP_CONCAT(
                            CONCAT(p.nombre, ' x', dv.cantidad, ' ($', FORMAT(dv.precio_unitario,2), ')')
                            SEPARATOR ', '
            ...
            if (empty($ventas)) {
                return ['success' => false, 'message' => "No se encontraron tickets que coincidan con \"$termino_limpio\""];
            }

            // Si hay exactamente una coincidencia, devolver con detalles completos directamente
            if (count($ventas) === 1) {
                $venta = $ventas[0];
            ...
            // Múltiples coincidencias → devolver lista para que el usuario elija
            return [
                'success' => true,
                'unico'   => false,
                'ventas'  => $ventas
    ...
    public function obtenerDetallesVenta($venta_id) {
        ...
        try {
            ...
            return ['success' => false, 'message' => 'Error al obtener detalles de la venta'];
        }
    }
}
?>