...
class ProductosAdmin {
    ...
    public function obtenerTodos() {
        ...
        try {
            $stmt = $conn->prepare("SELECT p.*, 
                                    (SELECT COUNT(*) FROM detalles_venta dv WHERE dv.producto_id = p.id) as ventas_asociadas
                                    FROM productos p 
                                    ORDER BY p.nombre");
            $stmt->execute();
    ...
    public function registrar($datos) {
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO productos (codigo_barras, nombre, descripcion, categoria, precio, stock_minimo, stock_actual) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?)");
            ...
            if ($stmt->execute()) {
                ...
                if ($stock_actual > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO movimientos_inventario (producto_id, tipo, cantidad, stock_anterior, stock_nuevo, justificacion) 
                                            VALUES (?, 'entrada', ?, 0, ?, ?)");
